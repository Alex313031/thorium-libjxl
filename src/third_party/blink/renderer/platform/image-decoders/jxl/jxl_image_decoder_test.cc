...
std::unique_ptr<ImageDecoder> CreateJXLDecoderWithArguments(
  ...
  scoped_refptr<SharedBuffer> data = ReadFileToSharedBuffer(jxl_file);
...
std::unique_ptr<ImageDecoder> CreateJXLDecoder() {
  return std::make_unique<JXLImageDecoder>(
      ...
      ColorBehavior::kTag, ImageDecoder::kNoDecodedImageByteLimit);
}
...
class PerByteSegmentReader : public SegmentReader {
 public:
  explicit PerByteSegmentReader(Vector<char> data) : data_(std::move(data)) {}
  size_t size() const override { return data_.size(); }
  base::span<const uint8_t> GetSomeData(size_t position) const override {
    if (position >= data_.size()) {
      return {};
    }
    return base::as_bytes(base::span(data_).subspan(position, 1u));
  }
...
void TestSegmented(const char* jxl_file, gfx::Size expected_size) {
  auto decoder = std::make_unique<JXLImageDecoder>(
      ImageDecoder::kAlphaNotPremultiplied, ImageDecoder::kDefaultBitDepth,
      ColorBehavior::kTag, ImageDecoder::kNoDecodedImageByteLimit);
  Vector<char> data = ReadFile(jxl_file);
  EXPECT_FALSE(data.empty());

  scoped_refptr<SegmentReader> reader =
      base::AdoptRef(new PerByteSegmentReader(std::move(data)));
...
TEST(JXLTests, PixelTest) {
  ...
  TestPixel("/images/resources/jxl/alpha-lossless.jxl", gfx::Size(2, 10),
            ...
            ImageDecoder::AlphaOption::kAlphaPremultiplied, ColorBehavior::kTag,
            0);
  ...
  TestPixel("/images/resources/jxl/3x3_srgb_lossless.jxl", gfx::Size(3, 3),
            ...
            ColorBehavior::kTransformToSRGB, 0);
  ...
  TestPixel("/images/resources/jxl/3x3_srgb_lossy.jxl", gfx::Size(3, 3),
            ...
            ImageDecoder::AlphaOption::kAlphaPremultiplied,
            ColorBehavior::kTransformToSRGB, 15);
  ...
  TestPixel("/images/resources/jxl/3x3_gbr_lossless.jxl", gfx::Size(3, 3),
            ...
            ImageDecoder::AlphaOption::kAlphaPremultiplied,
            ColorBehavior::kTransformToSRGB, 3);
  ...
  TestPixel("/images/resources/jxl/3x3_gbr_lossy.jxl", gfx::Size(3, 3),
            ...
            ImageDecoder::AlphaOption::kAlphaPremultiplied,
            ColorBehavior::kTransformToSRGB, 35);
...
TEST(JXLTests, ColorProfileTest) {
  TestColorProfile("/images/resources/jxl/icc-v2-gbr.jxl", ColorBehavior::kTag,
...
TEST(JXLTests, AnimatedPixelTest) {
  TestPixel(
      ...
      ImageDecoder::AlphaOption::kAlphaNotPremultiplied, ColorBehavior::kTag, 0,
      2);
}
...
TEST(JXLTests, JXLHDRTest) {
  ...
  TestHDR("/images/resources/jxl/pq_gradient_lossy.jxl", ColorBehavior::kIgnore,
          false, 0.58039218187332153, 0.73333334922790527, 0.43921568989753723,
          1);
  // sRGB as expected, but not an exact match
  TestHDR("/images/resources/jxl/pq_gradient_lossy.jxl",
          ColorBehavior::kTransformToSRGB, true, -0.9248046875, 1.943359375,
          -0.4443359375, 1);

  // linear sRGB as expected.
  TestHDR("/images/resources/jxl/pq_gradient_lossy.jxl", ColorBehavior::kTag,
  ...
  TestHDR("/images/resources/jxl/pq_gradient_lossless.jxl",
          ColorBehavior::kIgnore, false, 0.58039218187332153,
          0.73725491762161255, 0.45098039507865906, 1);
  TestHDR("/images/resources/jxl/pq_gradient_lossless.jxl",
          ColorBehavior::kTransformToSRGB, true, -0.95751953125, 1.9677734375,
          -0.416748046875, 1);
  // correct, original PQ values
  TestHDR("/images/resources/jxl/pq_gradient_lossless.jxl", ColorBehavior::kTag,
          true, 0.58056640625, 0.7373046875, 0.450927734375, 1);

  // with ICC
  // clipped linear sRGB, as expected from current JXL implementation
  TestHDR("/images/resources/jxl/pq_gradient_icc_lossy.jxl",
          ColorBehavior::kIgnore, false, 0, 0.0930381, 0, 1);

  TestHDR("/images/resources/jxl/pq_gradient_icc_lossy.jxl",
          ColorBehavior::kTransformToSRGB, false, 0, 0.338623046875, 0, 1);
  TestHDR("/images/resources/jxl/pq_gradient_icc_lossy.jxl",
          ColorBehavior::kTag, false, 0, 0.0930381, 0, 1);
  ...
  TestHDR("/images/resources/jxl/pq_gradient_icc_lossless.jxl",
          ...
          -0.416748046875, 1);
  TestHDR("/images/resources/jxl/pq_gradient_icc_lossless.jxl",
          ColorBehavior::kTag, true, 0.58039218187332153, 0.73725491762161255,
